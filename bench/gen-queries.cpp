...
int main(int argc, char** argv) {
  {
    ...
    if (options.file_sa.empty()) {
      options.file_sa =
          options.file_text + ".sa" + std::to_string(options.width);
    }
    if (options.file_lcp.empty()) {
      options.file_lcp =
          options.file_text + ".lcp" + std::to_string(options.width);
    }
  ...
  std::array<size_t, max_lcp_exp + 1> count;
  {
    for (size_t x = 0; x <= max_lcp_exp; x++) {
      auto outfile =
          std::filesystem::path(options.out_dir) / ("lce_" + std::to_string(x));
