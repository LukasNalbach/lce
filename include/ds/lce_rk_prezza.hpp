...
class lce_rk_prezza {
 public:
   ...
   */
  lce_rk_prezza() : n_{0}, text_{nullptr} {
  }

  template <typename C>
  lce_rk_prezza(C& container)
      : lce_rk_prezza(container.data(), container.size()) {
  }

  lce_rk_prezza(char_type* text, size_t size) 
      : n_{size}, text_{text} {
